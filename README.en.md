...
All SQL references use the prefixed physical name: `SELECT trxid ... WHERE trxid = #{trxid}` becomes `SELECT _trxid ... WHERE _trxid = #{trxid}`. Dameng rejects these special names as result aliases, including `_trxid AS "trxid"`, so existing `resultMap column="trxid"` declarations in `mapper-dm` are changed to `column="_trxid"`; Java properties and MyBatis parameters remain unchanged. SELECT, DML, dynamic SQL, and mixed-use local `<sql>` fragments all use the same physical-column rule.

When a query has no explicit `resultMap` and depends on `resultType` or `Map` automatic mapping, a directly returned special physical column is retained in the converted SQL and reported for manual review. The tool does not generate a potentially incomplete result map or change global MyBatis settings. `SELECT *` / `t.*` is not expanded; an explicit result map is still updated to use its physical database columns.
