...
- 判断上述变量能否跨越 `CALL` 时，不能把所有调用都视为未知副作用。同一脚本中调用前已定义、无参数且调用时无实参的过程，可分析其普通 `INSERT`、单表 `UPDATE`、单表 `DELETE` 及条件查询；确认不会修改变量来源表或会话变量后，允许继续内联。分析必须使用变量替换前的原始过程，并按调用位置跟踪 schema、定义、删除和重建，不能用后面的定义解释前面的调用，也不能混用不同 schema 的同名过程。过程含嵌套 CALL（包括递归）、动态 SQL、未知函数、会话设置、用户变量赋值或无法可靠分析的写入时仍视为未知；未解析调用还应使已缓存的过程定义失效。保留这些保护，并用“查询赋值后连续创建、调用、删除多个过程”的真实形态验证原有转换能力。
