...
- `ROWID`、`ROWNUM`、`TRXID`、`PHYROWID`、`VERSIONS_*` 等达梦伪列或特殊名称如果原本是业务表物理列，迁移命名统一增加前缀下划线，例如 `trxid` -> `_trxid`。Mapper 最外层显式查询投影应使用 `_trxid AS "trxid"` 保持原 JDBC 结果标签，避免修改既有 `resultMap column="trxid"`；WHERE、JOIN、DML、DDL、子查询和 CTE 内部使用前缀物理名。`SELECT *` / `t.*` 不自动展开。
