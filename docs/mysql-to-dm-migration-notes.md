...
- MySQL 过程最外层的 `EXIT HANDLER FOR SQLEXCEPTION BEGIN ... RESIGNAL; END` 只有在处理体不包含声明、嵌套控制流、事务控制或动态跳转，且过程内没有其他处理器、`RESIGNAL` 或达梦异常块时，才可改写为过程末尾的 `EXCEPTION WHEN OTHERS THEN ... RAISE`。清理语句必须先经过现有 SQL 规则转换；无法证明处理器作用域和退出后重抛语义等价时仍保留原文并人工确认。
