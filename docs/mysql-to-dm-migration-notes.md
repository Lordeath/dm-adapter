...
- MySQL 行级历史触发器只有在完整匹配“单个 `IF`、可选 `ELSE`、每个分支恰好一个 `INSERT ... VALUES`”的安全结构时，才使用规则 `MYSQL_SIMPLE_ROW_HISTORY_TRIGGER_TO_DM` 自动转换：`CREATE TRIGGER` 改为 `CREATE OR REPLACE TRIGGER`，代码区的 `OLD.col`/`NEW.col` 改为达梦的 `:OLD.col`/`:NEW.col`，`!=` 规范为 `<>`。字符串、注释和引用标识符不得改写；INSERT 事件不得引用 OLD，DELETE 事件不得引用 NEW，列值数量必须一致。包含变量、游标、循环、动态 SQL、异常处理、多个业务语句或不平衡块的触发器必须保留原文并继续人工确认。存储对象结束符 `/` 由脚本渲染器统一生成，规则本身不得重复拼接。
...
- MySQL `REGEXP`/`NOT REGEXP` 操作符应改写为达梦 `REGEXP_LIKE`。右侧表达式如果已经是达梦可执行的 `CONCAT(...)`，不需要额外转成 `||`。动态 `<foreach>` 构造模式时还要检查单元素分支：本地 DM8 已验证 `CONCAT(expr)` 会报 `-2207`，因此 `CONCAT(prefix, CONCAT(<foreach>...</foreach>), suffix)` 应在保持参数顺序的前提下去掉内层单参数 `CONCAT`，由外层多参数 `CONCAT` 同时覆盖一个和多个候选值。
...
- 动态 `INSERT [IGNORE] ... SELECT` 在目标列和 SELECT 投影可逐项配对且已配置方法键时，可使用有序游标逐行执行 `MERGE`，从而保留同一来源中重复键的处理顺序。SQL 脚本顶层或存储过程内的静态 `INSERT ... SELECT ... ON DUPLICATE KEY UPDATE` 也使用这一策略，但只能按每条 INSERT 的目标表读取无歧义的 table-level `keyColumns`；过程内不能复用前一条 INSERT 的表或键。配置键必须完整出现在 INSERT 列清单中，目标列与 SELECT 投影数量必须一致；`column = column`、`column = table.column` 这类 no-op 更新应省略 `WHEN MATCHED`。原查询尾部带 `FOR UPDATE` 时，达梦不允许聚合游标继续保留该子句；工具仅在来源是可识别的单表时改为匿名块内 `LOCK TABLE 来源表 IN SHARE MODE` 后移除 `FOR UPDATE`，以保护读取期间的来源数据。来源为派生表、复杂表达式或无法提取锁表对象时保留原 SQL 并报告，不能静默去锁。
...
- MySQL 的 `ALTER TABLE ... MODIFY/CHANGE` 会用完整列定义替换旧定义：没有写 `NOT NULL` 就表示字段可空。达梦的 `MODIFY` 如果省略 `NULL`/`NOT NULL`，则会保留字段原有的可空约束；`DEFAULT NULL` 只设置默认值，不能解除 `NOT NULL`。因此转换 MySQL 完整列定义时，源定义显式 `NULL` 或 `NOT NULL` 应原样保留，二者都未写时必须在达梦输出中补充显式 `NULL`，例如 `VARCHAR(255 CHAR) NULL DEFAULT NULL`。
...
- MySQL 过程最外层的 `EXIT HANDLER FOR SQLEXCEPTION BEGIN ... RESIGNAL; END` 只有在处理体不包含声明、嵌套控制流、事务控制或动态跳转，且过程内没有其他处理器、`RESIGNAL` 或达梦异常块时，才可改写为过程末尾的 `EXCEPTION WHEN OTHERS THEN ... RAISE`。事务处理的明确例外是处理体仅有 `ROLLBACK; RESIGNAL;`：可转换为 `EXCEPTION WHEN OTHERS THEN ROLLBACK; RAISE;`，保留回滚和原错误传播；注释应原样保留。处理器必须位于过程外层声明位置，不能把内层处理器提升到整个过程；`ROLLBACK TO SAVEPOINT`、回滚后额外清理或提交等其他事务组合仍须人工确认。清理语句必须先经过现有 SQL 规则转换；无法证明处理器作用域和退出后重抛语义等价时仍保留原文并人工确认。依赖 CALL 的提示应要求完成达梦适配，不能把尚未支持的转换表述成原始 SQL 错误。
- MySQL 过程中的无附加选项 `START TRANSACTION;` 会隐式提交当前事务；达梦在 CALL 已开启事务时直接执行它会报 `-6510`。规则 `MYSQL_PROCEDURE_START_TRANSACTION_TO_DM` 在原控制流位置补充 `COMMIT;`，并保留后面的 `START TRANSACTION;`，既结束之前的事务，也显式开始新的事务；不能仅删除事务开始语句。前面已有独立 `COMMIT;` 时不重复补充，字符串和注释中的事务文字不改写。带 `READ ONLY`、`WITH CONSISTENT SNAPSHOT` 等选项时继续人工确认。2026-09-09 在本地 DM8 20260414 构建的独立测试实例中验证了异常后全部更新回滚、用户异常和唯一键异常继续抛出、正常执行提交，以及事务开始前调用方的未提交数据按 MySQL 语义提交。
- 本地同一 DM8 构建中，`ROW_COUNT()` 在 `COMPATIBLE_MODE=0` 的过程内无法解析，在 `COMPATIBLE_MODE=4` 下可以执行，默认按 MySQL 兼容模式保留原函数，不应因非兼容模式的失败而统一替换为 `SQL%ROWCOUNT`。两种达梦写法在原值更新的探针中都计为 1；业务依赖 MySQL 默认的“实际变更行数”时仍要单独核对源连接的 `CLIENT_FOUND_ROWS` 和重复调用的计数语义，不能声称达梦命中行数与 MySQL 变更行数完全等价。
...
- 升级脚本可能由 Windows 工具保存为 GB2312、GBK、GB18030、Big5 或 UTF-16/32，而不只是 UTF-8。SQL 转换和项目 DDL 元数据扫描必须使用同一个编码读取器，逐文件识别；优先识别 BOM（UTF-32 必须先于 UTF-16），再识别无 BOM Unicode 和旧编码，本次生成的输出统一为无 BOM UTF-8。短中文文本需要避免通用检测器误判成其他语言编码；无 BOM 旧编码存在歧义时可用 `DM_SQL_SCRIPT_FALLBACK_ENCODING` 或 `dm.adapter.sqlScriptFallbackEncoding` 指定非 Unicode 文件的实际编码。所有解码路径必须严格拒绝非法字节，日志和迁移摘要要保留文件路径、编码与底层错误原因，不能静默替换成乱码或只报告 `Input length = 1`。源文件和人工维护的保留脚本不覆盖。
- 达梦 JDBC 能执行超长字符串 SQL，不代表旧版 DIsql 通过 `START`/`@文件` 执行时也能接收同一条超长输入；单行 `UPDATE ... SET column = '<超长内容>'` 仍可能报 `DISQL-10033: 输入过长`。对最终达梦脚本中 UTF-8 超过 3000 字节、且能明确证明位于顶层 `UPDATE SET`、`INSERT VALUES`、普通 `INSERT ... SELECT` 直接投影或 `MERGE` 直接赋值位置的字符串，dm-adapter 使用规则 `DM_DISQL_LONG_DML_LITERAL_TO_CLOB_BLOCK` 生成匿名块：每段最多 900 UTF-8 字节，通过 `TO_CLOB` 逐段拼接后再执行原 DML。存储过程内已声明的 CLOB 直接赋值、已知 JSON/CLOB 过程参数，以及 `UPDATE SET`、`INSERT VALUES`、`INSERT ... SELECT` 直接投影和 `MERGE` 直接赋值，可使用规则 `DM_PROCEDURE_LONG_LITERAL_TO_CLOB_VARIABLE` 在原控制流位置生成局部 CLOB 变量并按相同规则分段拼接；未知过程参数不得猜测为大字段。存储过程内 `EXECUTE IMMEDIATE '<超长 SQL>'` 的直接字符串参数若 UTF-8 不超过 32767 字节，可使用规则 `DM_PROCEDURE_LONG_DYNAMIC_SQL_TO_VARCHAR_VARIABLE` 声明 `VARCHAR(32767)` 局部变量，并在原控制流分支内按最多 900 UTF-8 字节逐段赋值后执行，避免把不同分支的动态 SQL 提前初始化。超过 32767 字节、由表达式动态拼接或无法确认 `INTO`/`USING` 边界的动态 SQL 仍须人工处理。所有转换只折叠 SQL 的成对单引号，必须逐字符保留中文、emoji、单引号和反斜杠；原脚本不覆盖，只修改输出目录。其他位于 `WHERE`、函数表达式、`RETURNING` 等不安全位置的长字符串，以及单个超过 20MB 的字符串必须保留原 SQL 并进入人工确认，不能为了绕过客户端长度限制强行改写语义。
- 顶层 `INSERT ... SELECT` 的长字符串拆分支持有/无 `FROM`、多个直接字面量及明确的投影别名，必须以查询顶层 `FROM`、`WHERE` 等子句确定投影边界；字符串、注释和嵌套查询中的关键字不得干扰识别。只替换字面量，保留别名、来源与 `WHERE NOT EXISTS` 等条件。包含去重、分组、集合运算、排序、锁定或 `RETURNING` 的复杂形态，以及长字符串位于函数、条件或子查询中的情况，继续人工确认；不得部分转换后消除告警。
...
- 判断上述变量能否跨越 `CALL` 时，不能把所有调用都视为未知副作用。支持同一 schema 中调用前已定义的无参数过程（`CALL p`、`CALL p()`）、仅 IN 参数的过程和可解析的嵌套调用；按原始 SQL 分别记录写入的表和用户变量。普通 DML、静态 CREATE/ALTER TABLE、CREATE INDEX、局部变量及无关用户变量赋值可分析；JOIN UPDATE 保守记录所有涉及的来源/目标表，DDL 同时使相关元数据查询失效。已确认调用会修改本次变量或其查询来源表时禁止内联；只有缺少外部过程定义、且没有已识别副作用时，查询赋值变量沿用兼容行为，不新增人工确认，也不清空已经解析的本地过程定义。通过 `--sql-procedure-source` / Batch `sql.procedureSources` 可显式提供与实际部署一致的公共过程定义以增强分析，文件只读，不自动扫描历史目录、执行或部署。定义按调用位置跟踪 schema、删除和重建；嵌套调用必须使用调用时生效的定义，不能使用后面的定义解释前面的调用。跨文件保留定义失效状态，应用脚本与系统脚本上下文隔离。显式删除或失效的定义、递归、动态 SQL、未知函数、OUT/INOUT 参数、会话设置或无法可靠分析的写入仍视为阻断项；普通表达式变量仍严格检查未知 CALL。报告必须指出实际阻断变量转换的具体 CALL 和原因，不能对配置变量统一建议 ROW_NUMBER。
- 脚本级单变量 `SET @name = expression` / `:=` 可按 `MYSQL_SCRIPT_EXPRESSION_USER_VARIABLE_INLINE` 内联确定性表达式：支持常量、NULL、已解析变量、括号及 `CONCAT`、`COALESCE`、`NULLIF`、`IFNULL` 的组合。查询依赖仅复用已通过稳定标量查询分析的变量，并检查其表达式中没有未知函数、时间或随机值；不据此自动接受任意内嵌子查询。依赖值按赋值顺序捕获，后续重新赋值不能改变先前的结果。来源表发生修改、未知 CALL 位于赋值与最后使用之间、后续使用进入存储对象或未分析的控制流，以及自引用或未初始化变量时，继续人工确认。最后一次使用为 CALL 参数时可转换，必须保留展示 SELECT、CALL 及赋值审计空操作。稳定性判断限于脚本语句顺序，不提供并发写入下的一致性保证，也不替业务处理库名与模式映射。
...
- 同一动态 Mapper 方法含多条普通 DML 时，只有在分号位于 SQL 字符串、注释、括号及完整 XML 节点之外，且每条首关键字明确为 `INSERT`、`UPDATE`、`DELETE` 或 `SELECT` 时，才按原顺序逐条转换并汇总报告。单条 JOIN UPDATE 的安全判断与告警清理不得扩大到整个方法：一条成功转换不能清除另一条仍未解决的风险。字符串、XML 属性和实体中的分号不能当作 SQL 分隔符；包含 `${}`、未展开的 `<include>`、CDATA、动态生成分隔符或存储块时不启用这条拆分路径，继续采用原有保守处理。拆分不改变执行顺序、参数绑定或事务边界。
...
- `resultMap` 的 `column` 只表示查询结果集中的列标签，可能来自物理列、表达式别名或联表字段，不能用于推断动态 `UPDATE SET` 左侧的目标列。SET 左侧应保留原始 mapper 明确写出的数据库列名；即使 `property` 与 `column` 仅存在驼峰和下划线差异，也不得据此自动改写。若原始 mapper 确实把 Java 属性名误写成数据库列名，应修正原 mapper，而不是由 dm-adapter 根据读映射猜测写入列。
