...
- 判断上述变量能否跨越 `CALL` 时，不能把所有调用都视为未知副作用。支持同一 schema 中调用前已定义的无参数过程（`CALL p`、`CALL p()`）、仅 IN 参数的过程和可解析的嵌套调用；按原始 SQL 分别记录写入的表和用户变量。普通 DML、静态 CREATE/ALTER TABLE、CREATE INDEX、局部变量及无关用户变量赋值可分析；JOIN UPDATE 保守记录所有涉及的来源/目标表，DDL 同时使相关元数据查询失效。仅在调用不会修改本次变量或其查询来源表时继续内联。通过 `--sql-procedure-source` / Batch `sql.procedureSources` 可显式提供与实际部署一致的公共过程定义，文件只读，不自动扫描历史目录、执行或部署。定义按调用位置跟踪 schema、删除和重建；嵌套调用必须使用调用时生效的定义，不能使用后面的定义解释前面的调用。跨文件保留定义失效状态，应用脚本与系统脚本上下文隔离。递归、动态 SQL、未知函数、OUT/INOUT 参数、会话设置或无法可靠分析的写入仍视为未知，并使缓存的过程定义失效。报告必须指出阻断变量转换的具体 CALL 和原因，不能对配置变量统一建议 ROW_NUMBER。
