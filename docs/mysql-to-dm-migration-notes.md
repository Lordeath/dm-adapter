...
- 临时迁移过程中的元数据检查必须绑定 CLI `--schema` 指定的目标 schema，不能在过程内部用 `SYS_CONTEXT('USERENV','CURRENT_SCHEMA')` 推断。达梦执行存储过程时该上下文可能变为过程定义者的默认 schema，使 `ALL_TAB_COLUMNS`、`ALL_IND_COLUMNS` 等存在性检查查错对象并重复执行 DDL。
