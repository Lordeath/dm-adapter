...
- MySQL `CONCAT` 支持多个参数，而目标达梦过程表达式只接受二元 `CONCAT`。过程中的多参数动态 SQL 拼接应按原参数顺序改成左结合的嵌套二元调用，例如 `CONCAT(a,b,c)` 改成 `CONCAT(CONCAT(a,b),c)`；不能保留多参数形式，也不能直接换成 NULL 语义不同的 `||` 后就声称等价。达梦过程解析器不能稳定接受嵌套 `CONCAT` 参数中的多行字符串字面量，因此字面量里的 CR/LF 还要等价拆成 `CHR(13)`/`CHR(10)` 参与二元拼接，不能简单删除换行并改变字符串内容。
