...
| D16 | `information_schema.tables` + `DATABASE()`                 | <br />          | 普通 SQL 使用 `ALL_TABLES` + `SYS_CONTEXT`；存储过程使用 `ALL_TABLES` + `SF_GET_SCHEMA_NAME_BY_ID(CURRENT_SCHID)` | 无 `information_schema` schema，`DATABASE()` 不可解析 |
