...
数据库验证的总时限由 `DM_SQL_VALIDATION_TOTAL_TIMEOUT_SECONDS` 控制，默认 `7200` 秒（2 小时），SQL 脚本验证和 Mapper 验证共享同一时限。SQL 脚本单条语句时限由 `DM_SQL_SCRIPT_VALIDATION_TIMEOUT_SECONDS` 控制，默认 `180` 秒；工具除设置 JDBC 查询超时外还会执行自身的硬超时，主动取消语句并停止当前验证，避免驱动忽略 `setQueryTimeout` 后无限挂起。Mapper 验证每累计 50 条记录会原子更新报告；超时或进程中断后可读取已完成部分。新一轮运行开始前，上一轮报告会保留为 `sql-validation-report.previous.md/json`。
